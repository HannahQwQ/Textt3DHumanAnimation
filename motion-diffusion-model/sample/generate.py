...
def main(args=None):
    ...
    #     out_path += f"_{timestamp}"

    if out_path == '':
        # 1. 输出根目录： ../save/mdm/
        output_root = os.path.abspath(os.path.join(os.getcwd(), '..', 'save', 'mdm'))

        # ------ 2. 构建子文件夹名（prompt + timestamp） ------
        if args.text_prompt != '':
            name = args.text_prompt.replace(' ', '_').replace('.', '')
        elif args.input_text != '':
            name = os.path.basename(args.input_text).replace('.txt', '').replace(' ', '_').replace('.', '')
        elif args.dynamic_text_path != '':
            name = os.path.basename(args.dynamic_text_path).replace('.txt', '').replace(' ', '_').replace('.', '')
        else:
            name = "motion"
        ...
        folder_name = f"{name}@{timestamp}"
         ...
         # 4. 正确构建完整路径（加入子文件夹）
        out_path = os.path.join(output_root, folder_name)
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            print(f"Sample {sample_i}, Repetition {rep_i}, Skeleton:", skeleton)
