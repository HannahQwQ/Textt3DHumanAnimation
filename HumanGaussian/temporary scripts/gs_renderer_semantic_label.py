...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        # -----------------------------------------------------
        # Load semantic labels from PLY (if exists)
        # -----------------------------------------------------
        if "semantic" in plydata.elements[0].data.dtype.names:
            semantics = np.asarray(plydata.elements[0]["semantic"])
            print(f"[load_ply] Loaded semantic labels: {semantics.shape}")

            # save as torch tensor
            self._semantics = torch.tensor(
                semantics, dtype=torch.long, device="cuda"
            )

            # assign distinct colors for each semantic label
            # Palette for 27 classes
            # SEM_COLORS = np.array([
            ...
            # semantic_colors = SEM_COLORS[semantics]       # shape [N,3]
            semantic_colors = np.zeros((len(semantics), 3))
            semantic_colors[:] = [0.6, 0.6, 0.6]  # 默认灰色
            semantic_colors[semantics == 12] = [1.0, 0.0, 0.0]  # 红色

            # store in model
            self.semantic_colors = torch.tensor(
                semantic_colors, dtype=torch.float32, device="cuda"
            )

            print(f"[load_ply] semantic_colors assigned: {self.semantic_colors.shape}")
        else:
            print("[load_ply] No semantic labels found in PLY.")
...
class Renderer:
    ...
    def render(
        ...
        # use semantic labels to render color, visualise if each part is seperate correctly.
        # --- Semantic Color Override ---
        if override_color is not None:
            # override_color: [N, 3], tensor
