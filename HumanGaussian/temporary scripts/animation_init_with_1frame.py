...
class Skeleton:
    def __init__(self, opt):
        ...
        # natural init will be taken from motion file if available
        ...
        # motion/param data
        self.motion_seq = None  # [T, 21, 3] axis-angle of SMPLX body joints
        self.global_orient_seq = None  # [T, 1, 3]
        self.global_orient = None  # [1, 3]
        self.transl = None  # [3]
        self.betas = None
        self.expression = None

        if opt.motion is not None:
            _data = np.load(opt.motion)
            if "poses" in _data:
                poses = _data["poses"]  # expect [T, 22, 3] for SMPLX body (incl. root)
                if poses.ndim == 3 and poses.shape[1] >= 22 and poses.shape[2] == 3:
                    self.global_orient_seq = poses[:, 0:1]  # [T,1,3]
                    self.motion_seq = poses[:, 1:22]  # [T,21,3]
                elif poses.ndim == 2 and poses.shape[-1] % 3 == 0:
                    # flattened axis-angle, try to reshape
                    K = poses.shape[-1] // 3
                    poses = poses.reshape(-1, K, 3)
                    if K >= 22:
                        self.global_orient_seq = poses[:, 0:1]
                        self.motion_seq = poses[:, 1:22]
                # initialize from first frame if parsed
                if self.motion_seq is not None and len(self.motion_seq) > 0:
                    self.body_pose = np.array(self.motion_seq[0])
                if self.global_orient_seq is not None and len(self.global_orient_seq) > 0:
                    self.global_orient = np.array(self.global_orient_seq[0, 0])  # [3]

            # optional static parameters
            if "betas" in _data:
                self.betas = _data["betas"]
            if "expression" in _data:
                self.expression = _data["expression"]
            if "transl" in _data:
                self.transl = _data["transl"] if _data["transl"].ndim == 1 else _data["transl"][0]
    ...
    def load_smplx(self, path, betas=None, expression=None, gender="neutral"):
        ...
        # resolve model root path with fallbacks
        def _normalize_root(p):
            if p is None:
                return None
            # support both parent-of-"smplx" and "smplx" folder itself
            if os.path.basename(p) == "smplx":
                return os.path.dirname(p)
            if os.path.isdir(os.path.join(p, "smplx")):
                return p
            if os.path.isfile(os.path.join(p, "SMPLX_NEUTRAL.npz")):
                # p is the smplx folder itself (contains npz files)
                return os.path.dirname(p)
            return p

        def _contains_models(root):
            if root is None:
                return False
            return os.path.isfile(os.path.join(root, "smplx", "SMPLX_NEUTRAL.npz"))

        model_root = _normalize_root(path)

        if not _contains_models(model_root):
            # try environment variable
            env_p = os.environ.get("SMPLX_PATH", None)
            model_root = _normalize_root(env_p)

        if not _contains_models(model_root):
            # try common local locations relative to cwd
            candidates = [
                os.path.join(os.getcwd(), "models"),
                os.path.join(os.getcwd(), "models", "models_smplx_v1_1", "models"),
                os.path.join(os.getcwd(), "smplx"),
                os.path.join(os.path.dirname(os.getcwd()), "models"),
            ]
            for c in candidates:
                test_root = _normalize_root(c)
                if _contains_models(test_root):
                    model_root = test_root
                    break

        if not _contains_models(model_root):
            raise FileNotFoundError(
                "Cannot locate SMPL-X models. Please pass --smplx_path, or set SMPLX_PATH env, "
                "pointing to a directory whose 'smplx' subfolder contains SMPLX_*.npz."
            )
        ...
        # ensure torch tensors for smplx inputs
        body_pose_t = torch.tensor(self.body_pose, dtype=torch.float32).unsqueeze(0)
        lhand_t = torch.tensor(self.left_hand_pose, dtype=torch.float32).unsqueeze(0)
        rhand_t = torch.tensor(self.right_hand_pose, dtype=torch.float32).unsqueeze(0)
        ...
        betas_t = _to_tensor_2d(betas if betas is not None else self.betas)
        expr_t = _to_tensor_2d(expression if expression is not None else self.expression)
        global_orient_t = None if self.global_orient is None else torch.tensor(self.global_orient, dtype=torch.float32).unsqueeze(0)
        transl_t = None if self.transl is None else torch.tensor(self.transl, dtype=torch.float32).unsqueeze(0)

        smplx_output = self.smplx_model(
            body_pose=body_pose_t,
            left_hand_pose=lhand_t,
            right_hand_pose=rhand_t,
            betas=betas_t,
            expression=expr_t,
...
class GUI:
    ...
    def step(self):
        ...
        if self.playing:
            ...
            if self.skel.global_orient_seq is not None:
                self.skel.global_orient = np.array(self.skel.global_orient_seq[self.seq_id % len(self.skel.global_orient_seq), 0])
            self.seq_id += 1
...
if __name__ == "__main__":
    ...
    if opt.motion is not None:
        ...
        name = os.path.splitext(os.path.basename(opt.ply))[0]
