...
def joint_mapper_smplx_to_21(joints):
    indices = (
        np.array(
            [
                1,  # 
...
class Skeleton:
    def __init__(self, opt):
        ...
        # 更放松的自然姿态
        # self.body_pose[15, 2] = -0.3
        # self.body_pose[16, 2] = 0.3
        # self.body_pose[0, 1] = 0.05
        # self.body_pose[1, 1] = -0.05
        ...
        # add learnable residual to gauss & mesh mapping
        self.residual_uv = None  # [N, 2]

        # add ankle radius to module 
        self.joint_radius_dict = {
            3 : 0.50,   #left_knee
            4 : 0.50,   #right_knee
            6 : 0.25,
            7 : 0.25,
    ...
    def compute_joint_spheres(self):
        """
        joint_radius_dict: dict
            key: joint number (str), must be in self.bodypose
            value: radius (float), in same unit as points3D (meters)

        return:
            dict: {joint_idx: (center_xyz, radius)}
        """
        joint_spheres = {}

        for joint_id, radius in self.joint_radius_dict.items():
            # assert joint_id in self.drive_joints_3d, f"Unknown joint: {joint_id}"

            # joint position (xyz)
            center = self.drive_joints_3d[joint_id, :3].copy()

            joint_spheres[joint_id] = (center, radius) 
        
        # DEBUG: print first few joint spheres
        # print("==== DEBUG joint_spheres ====")
        # for i, (joint_id, (center, radius)) in enumerate(joint_spheres.items()):
        #     if i >= 5:
        #         break
        #     print(
        #         f"joint {joint_id}: "
        ...
        return joint_spheres

    def query_gaussians_in_sphere(self, center, radius):
        """
        center: np.ndarray, shape [3]
        radius: float

        return:
            np.ndarray of gaussian indices
        """
        # Gaussian centers: [N, 3]
        xyz = self.gs.gaussians.xyz
    ...
    def joint_motion_energy(self, joint_idx, frame_idx, window_size=5):
        """
        joint_idx: int
        frame_idx: int
        window_size: int (half window)
        ...
        poses = self.motion_seq[start : frame_idx + 1, joint_idx]  # [W, 3]

        # frame-to-frame angular velocity
        diffs = poses[1:] - poses[:-1]  # [W-1, 3]
    ...
    def load_smplx(self, path, betas=None, expression=None, gender="neutral"):
        ...
        drive_joints = smplx_output.joints.detach().cpu().numpy()[0]  # [127, 3]
        drive_joints = joint_mapper_smplx_to_21(drive_joints)

        self.drive_joints_3d = np.concatenate(
            [drive_joints, np.ones_like(drive_joints[:, :1])], axis=1
        )  # [21, 4]

        self.drive_joints_3d[:, :3] = (
            self.drive_joints_3d[:, :3] - self.ori_center
        ) * self.ori_scale

        self.scale_drive(-10)

        self.joint_spheres = self.compute_joint_spheres()
        ...
        if self.mapping_face is None:
            ...
            # initialize in-plane residual (MaGS-style, minimal)
            self.residual_uv = np.zeros((self.mapping_face.shape[0], 2), dtype=np.float32)


        else:
            # ===============================
            # 1. numpy -> torch（统一到 GPU）
            # ===============================
            device = self.gs.gaussians.get_xyz.device

            faces = self.faces[self.mapping_face]
            v0 = torch.from_numpy(self.vertices[faces[:, 0]]).to(device)
            v1 = torch.from_numpy(self.vertices[faces[:, 1]]).to(device)
            v2 = torch.from_numpy(self.vertices[faces[:, 2]]).to(device)

            mapping_uvw = torch.from_numpy(self.mapping_uvw).to(device)
            mapping_dist = torch.from_numpy(self.mapping_dist).to(device)

            # ===============================
            # 2. face normal & closest point
            # ===============================
            fnormals = torch.cross(v1 - v0, v2 - v0, dim=1)
            fnormals = fnormals / (torch.norm(fnormals, dim=1, keepdim=True) + 1e-20)
            ...
            # ===============================
            # 3. face tangent directions
            # ===============================
            t1 = v1 - v0
            t1 = t1 / (torch.norm(t1, dim=1, keepdim=True) + 1e-20)

            t2 = torch.cross(fnormals, t1, dim=1)
            t2 = t2 / (torch.norm(t2, dim=1, keepdim=True) + 1e-20)

            # ===============================
            # 4. residual（转 torch）
            # ===============================
            residual = torch.from_numpy(self.residual_uv).to(device)  # [N,2]

            # ===============================
            # 5. 关节邻域 + 动作窗口调制
            # ===============================
            xyz_prev = self.gs.gaussians.get_xyz.detach()  # torch [N,3],高斯点在套用动作前的位置，要考虑多步怎么恢复高斯点的原来位置（可能需要单独存储）

            # 强行将高斯点推向mesh表面
            for joint_idx, (center_np, radius) in self.joint_spheres.items():
                center = torch.from_numpy(center_np).to(device)  # [3]
                dist = torch.norm(xyz_prev - center[None, :], dim=1)
                mask = dist < radius

                # debug
                # num = mask.sum().item()
                # print(f"[DEBUG] joint {joint_idx}: selected gaussians = {num}")

                # if num > 0:
                #     # 打印前 5 个被选中的高斯点
                #     idx = torch.nonzero(mask, as_tuple=False).squeeze(1)[:5]
                #     pts = xyz_prev[idx].detach().cpu().numpy()

                #     print(f"  first {pts.shape[0]} gaussian xyz:")
                #     for i, p in enumerate(pts):
                #         print(f"    {i}: [{p[0]:.4f}, {p[1]:.4f}, {p[2]:.4f}]")

                #     # 同时打印它们到 joint center 的距离（很重要）
                #     d = dist[idx].detach().cpu().numpy()
                #     print(f"  distances to joint center: {d[:pts.shape[0]]}")

                # 先无条件处理
                residual[mask] *= 0.0   # ← 直接压扁切平面自由度


            # for joint_idx, (center_np, radius) in self.joint_spheres.items():
            #     energy = self.joint_motion_energy(
            #         joint_idx,
            #         self.frame_idx,
            #         window_size=5,
            #     )
            ...
            # 先显式的过度的修改一下mesh_distance
            mapping_dist_new = mapping_dist.clone()
            mapping_dist_new[mask] *= 0   # 或 + offset
            ...
            points = (
                cpoints
                + mapping_dist_new[:, None] * fnormals
                + residual[:, 0:1] * t1
                + residual[:, 1:2] * t2
            )

            # ===============================
            # 7. 关节区域：强制法线吸附（局部 override）
            # ===============================
            # if mask.any():
            #     push_strength = 1.5
            #     points[mask] = (
            #         cpoints[mask]
            #         + push_strength * mapping_dist[mask, None] * fnormals[mask]
            #     )

            self.gs.gaussians._xyz = points
    ...
    def scale_drive(self, delta):
        self.drive_joints_3d[:, :3] *= 1.1 ** (-delta)
        # if self.vertices is not None:
        #     self.vertices *= 1.1 ** (-delta)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--smplx_path",
        type=str,
        default="./models/models_smplx_v1_1/models",
        help="path to models folder (contains smplx/)",
    )
    parser.add_argument(
        "--save", type=str, default="./saved/animated", help="path to render and save video"
    )
