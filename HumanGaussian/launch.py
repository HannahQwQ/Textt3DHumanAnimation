...
def main(args, extras) -> None:
    ...
    # ------------------ EXPORT TRIAL DIR ------------------
    if get_rank() == 0:
        export_dir = os.path.join(PROJECT_ROOT, "save", "HGmodel")
        os.makedirs(export_dir, exist_ok=True)

        export_path = os.path.join(export_dir, "last_trial.json")

        # 1. 将 cfg.trial_dir 解析为绝对路径
        trial_dir_abs = Path(cfg.trial_dir).resolve()

        # 2. 转换为相对于 PROJECT_ROOT 的路径（无 ../）
        trial_dir_rel = trial_dir_abs.relative_to(Path(PROJECT_ROOT))

        with open(export_path, "w") as f:
            json.dump(
                {
                    "trial_dir": str(trial_dir_rel),   # e.g. save/HGmodel/...
                    "exp_name": trial_dir_abs.name,
                    "config": args.config,
                },
                f,
                indent=2,
        ...
        print(f"[INFO] Exported trial_dir to {export_path}")
    # ------------------------------------------------------
