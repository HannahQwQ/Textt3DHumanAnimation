#origin render of gs color, without semantic lables
...
class Renderer:
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        # load checkpoint
        if input is None:
            # init from random point cloud 没给出初始的3DGS模型，随机初始化模型
            ...
            # load from a provided pcd 应对输入为点云的pcb格式文件
            self.gaussians.create_from_pcd(input, 1)
        else:
            # load from saved ply 应对一般的生成的3DGS人体模型的ply文件输入
