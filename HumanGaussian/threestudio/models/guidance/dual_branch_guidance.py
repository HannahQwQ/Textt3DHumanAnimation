...
class StableDiffusionGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        # Create model
        self.vae = self.pipe.vae.eval().to(self.device, dtype=self.weights_dtype)
        self.unet = self.pipe.unet.eval().to(self.device, dtype=self.weights_dtype)
    ...
    def encode_images(
        ...
        # print(f"[encode_images] input dtype: {imgs.dtype}, device: {imgs.device}")  # 输出输入信息
        input_dtype = imgs.dtype
        imgs = imgs * 2.0 - 1.0
        # posterior = self.vae.encode(imgs.to(self.weights_dtype)).latent_dist
        # latents = posterior.sample() * self.vae.config.scaling_factor
        # 不手动转换 dtype，让 AMP 自动处理
        posterior = self.vae.encode(imgs).latent_dist
        # print(f"[encode_images] posterior mean dtype: {posterior.mean.dtype}, device: {posterior.mean.device}")  # 输出 encode 后 dtype
        latents = posterior.sample() * self.vae.config.scaling_factor
        # print(f"[encode_images] latents dtype: {latents.dtype}, device: {latents.device}")  # 输出 latents dtype
